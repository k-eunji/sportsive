// src/app/ireland/dublin/sports/[date]/page.tsx
...
export async function generateMetadata(
  ...
  return {
    title: `Sports Fixtures in Dublin — ${displayDate} | VenueScope`,
    description:
      `Professional sports fixtures taking place in Dublin on ${displayDate}, organised by venue and start time.`,
    alternates: {
      canonical: `https://venuescope.io/ireland/dublin/sports/${date}`,
    },
  };
}
    ...
    <main className="max-w-3xl mx-auto px-6 py-16 space-y-8">
      ...
      <header className="space-y-4">
        <h1 className="text-3xl font-bold">
          Sports Fixtures in Dublin — {displayDate}
        </h1>

        <p className="text-muted-foreground">
          There are {dateEvents.length} professional sporting events
          taking place in Dublin on {displayDate}.
        </p>
      </header>
      ...
      <section className="pt-8">
        ...
        >
          All Ireland fixtures on {displayDate}
        </Link>

        <Link
          href={`/ireland/football/${date}`}
          className="underline underline-offset-4 block mt-2"
        >
          Football in Ireland on {displayDate}
        </Link>
