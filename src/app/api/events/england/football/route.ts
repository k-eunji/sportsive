...
export async function GET() {
  try {
    ...
    const { data: irelandMatches, error: irelandError } = await supabase
      .from("ireland_football")
      .select(`
        id,
        date,
        status,
      ...
      .order("date", { ascending: true });  
        ...
        if (irelandError) {
          console.error("Ireland matches error:", irelandError);
        }
    ...
    const formattedIrelandMatches = (irelandMatches || []).map((m: any) => ({
      id: `ire-${m.id}`,
      kind: m.kind,
      date: m.date,
      status: m.status,
