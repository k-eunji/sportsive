//src/app/sport/_components/HorseRacingCourseDetail.tsx
...
export default function HorseRacingCourseDetail({
  slug,
  events,
}: {
  slug: string;
  ...
  const all2026 = events.filter((e: any) => {
    const year = (e.startDate ?? "").slice(0, 4);
  ...
  const nationalShare =
    all2026.length > 0
      ? ((total / all2026.length) * 100).toFixed(1)
      : "0";
  ...
  const articleData = {
    ...
    description: `Structural analysis of race meetings held at ${venueName} during the 2026 horse racing season.`,
    author: {
      "@type": "Organization",
      name: "VenueScope",
    },
    ...
    <div className="max-w-4xl mx-auto px-6 py-14 space-y-12">
      ...
      <section className="text-sm text-muted-foreground leading-relaxed space-y-4">
        ...
        <p>
          ...
          concentration into a structural intensity score.
        </p>
      </section>
      ...
      <section className="text-sm text-muted-foreground leading-relaxed space-y-4">
        ...
        <p>
          With an average of <strong>{avgPerDay}</strong> meetings per active racing day,
          {venueName} demonstrates{" "}
          {avgPerDay > 1 ? "clustered scheduling" : "balanced distribution"}.
        </p>

        <p>
          A congestion score of <strong>{congestionScore}/100</strong> suggests
          {congestionScore > 60
        ...
        <p>
          Saturday concentration at <strong>{saturdayShare}%</strong>
          indicates
          {saturdayShare > 40
      ...
      <section className="text-sm text-muted-foreground leading-relaxed space-y-4 pt-6 border-t">
        ...
        <p>
          {venueName} accounts for <strong>{nationalShare}%</strong> of
          total meetings in the dataset.
        </p>
      ...
      <section className="pt-12 border-t space-y-6">
        ...
        <div className="space-y-5 text-sm text-muted-foreground">
          ...
          <div>
            ...
            <p className="mt-2">
              {venueName} hosts <strong>{total}</strong> meetings
